        ...
        grid.forEach(row => {
            ...
            if (left.join('') !== right.join('')) {
                for (let n=0; n < left.length; n++) {
                    if (left[n] !== right[n]) blemishes++;
                }
            }
        });

        if (blemishes === 1) count = m;
    }
    return count;
}
...
const findSymmetry = (pattern) => {
    ...
    //check for horizontal mirrors first
    const flipped = flipGrid(grid);
    let location = getMirrorLocation(flipped);
    if (location) {
        return location*100;
    } else {
        //then, flip grid back and check for vertical mirrors\
