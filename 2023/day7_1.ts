...
const getRank = (hand) => {
    const groups = hand.split('').sort().join('').match(/(.)\1*/g).sort((a,b) => b.length - a.length);
    if (groups.length === 5) {
        // high card
        return 0;
...
const getTieBreak = (card) => {
    return parseInt(card.replace('T', '10').replace('J', '11').replace('Q', '12').replace('K', '13').replace('A', '14'));
}
